            ...
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascipt:;" >
                  <span class="checkbox-btn item-check-btn" :class="{'checked':All==true}" @click="toggleAll()">
    ...
    export default {
        data() {
           return {
              ...
              modalConfirm:false,
              productId:'',
            //  All:false  计算属性不用在这里写，而且子计算属性里定义了的不需要在data中定义
           }
        },
        ...
        //货币格式化，过滤器的值为函数,通过对值处理，返回一个全新的值
        filters: {
          currency:currency//刚好其为一个函数

        },
       mounted() {
           this.init();
       },
      //计算属性一定有返回值,不会重复渲染，会自动计算
      computed: {
        All() {
      ...
      methods: {
         async init() {
            let res= await axios.get('/users/cartList');
            let  response=res.data;
                   this.cartList=response.result;
          },
        async delCart() {
              let res=await axios.post('/users/cartDel',{ productId:this.productId});
              let response=res.data;
                 let  ress=response.status;
                   if(ress=='0'){
                      this.modalConfirm=false;
                      this.init();
                      this.$store.commit("updateCartCount" , -response.result)
                  }
         },
       ...
       async  editCart(flag,item) {
            ...
            let  response= await axios.post('/users/cartEdit',{
       ...
       async  toggleAll() {
            // this.All=!this.All;  计算属性会实时计算，且不能改变数值，所以无用
               var  flag=!this.All;//flag可变
            this.cartList.forEach((item)=>{

                 item.checked=flag?'1':'0';
            })
         let response=await axios.post('/users/editCheckAll',{checkAll:flag});
             let res=response.data;
             if(res.status=='0'){
               console.log("切换成功");
             }
